...
/* ★NetAnalyzer 고성능 구성 (2026-07-30): AUTO -> HFXO 명시.
 *  AUTO 는 이 파일 위쪽에서 SL_CATALOG_RAIL_LIB_PRESENT 이면 HFXO 로 풀리므로
 *  현재도 결과는 같다. 다만 그 조건에 기대지 않고 **의도를 명시**한다 —
 *  본 장비는 비배터리 상시전원이라 SYSCLK/EM01GRPACLK/EM01GRPCCLK 를 전부
 *  39MHz HFXO(TCXO) 로 상시 구동하는 것이 설계 의도이고, 저전력 전환이 없다.
 *  근거: `작업지시_ClaudeCode_FG23_프로젝트생성.md` §2 (HFXO 상시 ON).
 *
 *  ※ 감지기(LTD_W10D_V03)와의 의도적 차이:
 *    V03 가이드 §4 는 EM01GRPACLK/EM01GRPCCLK 를 FSRCO 로 내려 HFXO 소비자를
 *    없앴다(저전력, 629µA 회귀 회피). 본 장비는 그 최적화를 **하지 않는다**.
 *    전류 제약이 없고, 클럭 전환 구간이 없어야 상시 RX 가 안정적이다.
 *
 *  [원본 -- SDK 기본값]
